...
public class RwandaMCHFlowsheetFormItem extends LinkExt {
	
	private final static Log log = LogFactory.getLog(RwandaMCHFlowsheetFormItem.class);
	...
	public String getLabel() {
		...
		if (this.getParameterMap() != null && this.getParameterMap().size() > 0){
			...
			if (!programFound)
				log.warn("MCHFlowsheet module: No valid programs were found in the global property mch.showPatientChartLinksIfInProgram.  This means that the MCH flowsheets will be available for NO patients.");
		}
